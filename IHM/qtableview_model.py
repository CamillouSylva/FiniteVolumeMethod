...
class PandasModelEditable(QAbstractTableModel):
    def __init__(self, data: list, table_header):
        super(PandasModelEditable, self).__init__()
    ...
    def columnCount(self, parent=QModelIndex()):
        return len(self._data[0])#len(self.header)
       ...
       # if index.isValid():
        if role == Qt.DisplayRole:
            ...
            if isinstance(value, float):
                ...
                return "%.3f" % value  # les valeurs sont arrondies à 4 chiffres après la virgule

            return value#str(self._data[index.column()])
